...
import login.oauthtest4.domain.RecStartData;
...
public class RecImgRegController {
    ...
    public byte[] uploadImageUrls(@RequestBody RecStartData request) {
        System.out.printf(request.getPaths().toString());
        System.out.printf("connect");
        ResponseEntity<byte[]> response = ai.recommand(request.getPaths());
...
//        }
        //return ai.recommand(request.getPaths());
    }
}